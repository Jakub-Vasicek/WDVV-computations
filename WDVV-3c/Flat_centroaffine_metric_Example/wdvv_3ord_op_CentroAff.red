...
% Finding the operator using the hypothesis that coordinates in which the 
% system is written are Casimirs of the operator.
% Here we use:
...
resname:="wdvv_3ord_op_CentroAff_res.red"$
...
shut "w3c_3ord_CentroAff_res.mw";

on nat;

gu3:=gl3**(-1)$

% Checking the Hamiltonian condition on the metric
operator equ2$
eqtel2:=0$
tempterm:=0$
dv0:={u1,u2,u3}$
 ...
 for n:=1:ncomp do
  for k:=1:ncomp do
   ...
   <<
    ...
    equ2(eqtel2:=eqtel2+1):=df(c_lo(m,n,k),part(dv0,l))+tempterm
   >>$
