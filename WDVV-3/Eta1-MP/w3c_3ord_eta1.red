...
% File eta1: finding the operator for eta case 1) from MP paper
...
% CDE is not necerssary here, just for covenience reasons
load_package cde$
...
% Filling the matrix with values for eta case 1) in MP paper
gw11:=0;
gw12:=0;
gw13:=1;
gw22:=rho;
...
% Checking the Hamiltonian condition
operator equ2$
eqtel2:=0$
tempterm:=0$
dv1:={u1_x,u2_x,u3_x}$
...
for m:=1:nc do
 for n:=1:nc do
  for k:=1:nc do
   for l:=1:nc do
   <<
    tempterm:=for p:=1:nc sum for q:=1:nc sum
...
shut "w3c_3ord_eta1_res.mw";
on echo$ on nat$

% Output for further computations with the metric
off exp; on gcd;
off echo$ off nat$
out <<resname>>;
